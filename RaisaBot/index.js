...
const start = async (tobz = new Client()) => {
        ...
        tobz.onAddedToGroup(async (chat) => {
            if(isWhite(chat.id)) return tobz.sendText(chat.id, 'Halo aku Kurimaya, Ketik #help Untuk Melihat List Command Ku...')
            if(mtcState === false){
                const groups = await tobz.getAllGroups()
                // BOT group count less than
                if(groups.length > groupLimit){
                    await tobz.sendText(chat.id, 'Maaf, Batas group yang dapat Kurimaya tampung sudah penuh').then(async () =>{
                    ...
                    if(chat.groupMetadata.participants.length < memberLimit){
                        ...
                        if(!chat.isReadOnly) tobz.sendText(chat.id, 'Halo aku Kurimaya, Ketik #help Untuk Melihat List Command Ku...')
                    }
                }
            }else{
                await tobz.sendText(chat.id, 'Kurimaya sedang maintenance, coba lain hari').then(async () => {
                    tobz.deleteChat(chat.id)
                    tobz.leaveGroup(chat.id)
                })
            }
